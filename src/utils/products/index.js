import { WorkType, workFactoryMapFormData } from "../../../src-ts/lib";
import {
...
const getPrizesAndTimeline = (workType) => {
    switch (workType) {
        case WorkType.data:
            return [getDataExplorationPriceAndTimelineEstimate(), dataExplorationTimeline];
        case WorkType.design:
            return [getWebsiteDesignPriceAndTimelineEstimate(), websiteDesignTimeline];
        case WorkType.findData:
...
const isDataScience = (workType) => {
    return [WorkType.data, WorkType.findData, WorkType.problem].includes(workType);
}